...
class JobOfferRepoDB(JobOfferRepository):
    ...
    async def get_analytics(
        ...
        agg_stmt = period_stmt.with_only_columns(
            ...
            func.sum(case((JobOfferDB.has_response.is_(True), 1), else_=0)).label("response_count"),
            func.sum(case((JobOfferDB.has_interview.is_(True), 1), else_=0)).label("interview_count"),
        ).order_by(None)
