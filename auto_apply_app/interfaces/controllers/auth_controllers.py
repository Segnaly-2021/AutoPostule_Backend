...
class AuthController:
    ...
    verify_code_use_case: VerifyCodeUseCase
    ...
    async def handle_verify_code(self, email: EmailStr, code: str) -> OperationResult:
        """
        Verifies the 6-digit code. On success, the use case returns a LoginResponse
        (access_token + token_type) — the user is logged in implicitly.
        """
        try:
            request = VerifyCodeRequest(email=email, code=code)
            result = await self.verify_code_use_case.execute(request)

            if result.is_success:
                # Same shape as login — present as a LoginViewModel.
                view_model = self.presenter.present_login(result.value)
                return OperationResult.succeed(value=view_model)
    ...
    async def handle_resend_verification(self, email: EmailStr) -> OperationResult:
        try:
            request = ResendVerificationRequest(email=email)
            result = await self.resend_verification_use_case.execute(request)
