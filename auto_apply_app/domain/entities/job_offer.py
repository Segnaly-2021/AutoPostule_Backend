...
class JobOffer(Entity):
    ...
    # [NEW] To support "Apply All" correctly, we need to know which 
    # generation run this job belongs to, if JobSearch is long-lived.
    # If JobSearch is created fresh every time, search_id is enough.
    # run_id: Optional[UUID] = field(default=None) 

    def mark_as_generated(self, cover_letter_text: str) -> None:
    ...
    def complete_application(self) -> None:
        ...
        if self.status != ApplicationStatus.APPROVED:
             # Basic users might skip APPROVED state in code, but conceptually 
             # it's cleaner to always go Generated -> Approved -> Submitted
